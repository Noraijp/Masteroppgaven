...
class SimCrossSectionData:
	...
	def ALICE(self, foil, A, Z):
		...
		with open(filename) as f:
			...
			CS = np.genfromtxt(filename, delimiter=' ', usecols=[3], skip_header=56, skip_footer=(len(content_full)-len(content)))
			E_new = []; CS_new = []
			Z = ' ' + Z + ' '
			A = ' ' + A + ' '
	...
	def TALYS(self,foil, Z, A, file_ending='.tot.064'):
		# Z = 0XX, A=0XX
	...
	"""
	def EXFOR(self, reaction):
		filename = self.path + '/../EXFOR/Zink/' + reaction + '.txt'
		#print(filename)
		print(reaction)
		...
		if os.path.isfile(filename):
			...
			with open(filename) as f:
				...
				plt.errorbar(E, CS, marker='.', linewidth=0.001, xerr=dE, yerr=dCS, elinewidth=0.5, capthick=0.5, capsize=3.0, label=author )
				plt.legend()
				plt.show()
...
#SimCS.EXFOR('Zn(n,x)64Cu')
#SimCS.Tendl('Zn', '052', '025', file_ending='.064')
#SimCS.ALICE('Zn', '191', '77')



#reaction = 'Zn(n,x)64Cu'
...
#E, dE, CS, dCS, author = SimCS.EXFOR('Zn(n,x)64Cu')
#print(E)
#SimCS.TALYS('Zn', '026', '059')
