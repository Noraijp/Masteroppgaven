...
name_of_csv_file = './87Y_y33MeV_239.dat'
name_of_csv_file_parent = './87mY_y33MeV_239.dat'

### 66Ni/66Cu decay chain, units of hours, assumed 2:1 production rate, for t_irradiation hours
#
dc =      DecayChain('87Ym', 'h', R={'87Yg':1.0, '87Ym':10.0}, time=530.0/3600.0)
dc.append(DecayChain('87Ym', 'h',                             time=((5.0*60.0)+40.0)/3600.0))
dc.append(DecayChain('87Ym', 'h', R={'87Yg':1.0, '87Ym':10.0}, time=6750.0/3600.0))
...
# outfile_daughter = np.array((EoB_activities[2], uncertainty_EoB_activities[2]))
# outfile_granddaughter = np.array((EoB_activities[3], uncertainty_EoB_activities[3]))


outfile_parent = np.array((EoB_activities[0], 0.05*EoB_activities[0]))
outfile_daughter = np.array((EoB_activities[2], 0.05*EoB_activities[2]))
outfile_granddaughter = np.array((EoB_activities[3], 0.05*EoB_activities[3]))

#outfile = np.transpose(np.column_stack((EoB_activities, uncertainty_EoB_activities)))

#print(outfile_parent)
np.savetxt("../find activity/activity_csv/Y_87mY.csv", np.transpose(outfile_parent), delimiter=",")
np.savetxt("../find activity/activity_csv/Y_87Y.csv", np.transpose(outfile_daughter), delimiter=",")
np.savetxt("../find activity/activity_csv/Y_87mSr.csv", np.transpose(outfile_granddaughter), delimiter=",")
