...
def beam_current(A0, rho_dr, lambdas, t_irradiation, reaction_integral):
	return (1.6E-10 * 1E27 * A0) / (rho_dr * (1 - np.exp(-lambdas*t_irradiation)) * reaction_integral)
...
def sigma_I_approximate(A0, rho_dr, lambdas, t_irradiation, reaction_integral, unc_A0, unc_rho_dr, unc_lambdas, unc_t_irradiation, unc_reaction_integral):
	approx_error = np.sqrt(np.power(dIdA0(A0, rho_dr, lambdas, t_irradiation, reaction_integral) * unc_A0,2) + np.power(dIdRhoDr(A0, rho_dr, lambdas, t_irradiation, reaction_integral) * unc_rho_dr,2) + np.power(dIdLambda(A0, rho_dr, lambdas, t_irradiation, reaction_integral) * unc_lambdas,2) + np.power(dIdTIrradiation(A0, rho_dr, lambdas, t_irradiation, reaction_integral) * unc_t_irradiation,2) + np.power(dIdIntegral(A0, rho_dr, lambdas, t_irradiation, reaction_integral) * unc_reaction_integral,2))
	# approx_error = 0
...
# All in nuclei / cm^2
#                       Cu       Ti
molar_mass = np.array([63.546, 47.867])
areal_density = read_csv('./rho_drs.csv') * 6.022E20 / molar_mass
areal_density = areal_density[:, np.r_[1, 1, 0, 0]]
uncertainty_areal_density = read_csv('./unc_rho_drs.csv') * 6.022E20 / molar_mass
uncertainty_areal_density = uncertainty_areal_density[:, np.r_[1, 1, 0, 0]]

# All in Bq
#                      Sc46    V48    Zn62    Zn63
EoB_activities = read_csv('./A0.csv') 
uncertainty_EoB_activities = read_csv('./unc_A0.csv') 
# Normalized integral(sigma * dPhidE * dE)
# 
reaction_integral = read_csv('./rxn_integral.csv') 
# All in 1/s
lambdas = read_csv('./lambda.csv') 
# print(type(lambdas))
# All in s
t_irradiation = read_csv('./delta_t.csv') 
uncertainty_t_irradiation = 2
...
corr_EoB_activities = 0.3 * np.ones((number_of_monitor_reactions,number_of_monitor_reactions))
...
# Get correlation submtarix for each monitor foil - n x n, where n= # of rxns per foil 
for i in range(0, number_of_monitor_foils):
...
for i_energy in range(0, number_of_energies):
	...
	unc_delta_t = np.ones(number_of_monitor_reactions) *uncertainty_t_irradiation
	percent_rn_uncertainties = np.array([0.051054188386, 0.064100768909, 0.084213661384, 0.04385708308])
	unc_rxn_int = rxn_int * 	percent_rn_uncertainties
	...
	temp_currents =  beam_current(A0, ad, loop_lambdas, delta_t, rxn_int)
	currents[i_energy, :] =  temp_currents
	unc_temp_currents = sigma_I_approximate(A0, ad, loop_lambdas, delta_t, rxn_int, unc_A0, unc_ad, uncertainty_lambdas, unc_delta_t, unc_rxn_int)
	...
	for i_index,i_element in enumerate(nonzero_indices[0]):
		for j_index,j_element in enumerate(nonzero_indices[0]):
			numerator += temp_currents[0,j_element] * inverted_covariance[i_index,j_index]
...
plt.legend(['Actual Currents', 'Individual Currents'],loc='lower left')
