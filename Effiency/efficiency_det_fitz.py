...
class Efficiency_calculations(Detector_Information_fitz):
    ...
    def __init__(self, detector):
        ...
        list_detector = ['room131_5', 'room131_10', 'room131_18', 'room131_22'] # ['HPGE', 'HPGE2', 'IDM1', 'IDM2', 'IDM3', 'IDM4', 'room_131_5cm', 'room_131_10cm', 'room_131_15cm', 'room_131_18cm', 'room_131_22cm', 'room_131_30cm', 'room_131_40cm', 'room_131_50cm', 'room_131_60cm']
        ...
        self.t_half=np.array((30.08, 10.551, 13.517))*365*24*3600 # seconds, Cs137, Ba133, Eu152
        self.t_half_uncert = np.array((0.09, 0.011, 0.014))*365*24*3600

        self.lamb =np.log(2)/self.t_half
        self.sigma_lamb = self.lamb*self.t_half_uncert/self.t_half

        self.A_0 = np.array((38.55, 39.89, 39.29))*1e3 # Source activity Bq, Cs,Ba,Eu
        ...
        def information_from_files(filename):
            channel=np.loadtxt(filename, skiprows=28,usecols=[1])
            Nc=np.loadtxt(filename, usecols=[5], skiprows=28) #net area
            sigma_Nc=np.loadtxt(filename, usecols=[6], skiprows=28)
            E=np.loadtxt(filename, usecols=[0], skiprows=28)
        ...
        print(self.A_0[0])
        print(self.I_Cs137)
        print(self.live_time[0])
        print(self.time_delay[0])
    ...
    def plot_func(self):
        ...
        scipy.io.savemat('eff_{}.mat'.format(self.detector), {'popt_{}'.format(self.detector): popt, 'pcov_{}'.format(self.detector):pcov} )
        #scipy.io.savemat('eff_{}'.format(self.detector), {'popt_{}'.format(self.detector): popt, 'pcov{}'.format(self.detector):pcov})
        ...
        print("Efficiencies for hand calculations: {}".format(self.efficiency_estimated_popt([91.1, 93.2, 184.5],*popt)))
        ...
        plt.savefig("{}".format(self.detector), dpi=300)
        plt.show()
        plt.clf()
...
x = Efficiency_calculations('room131_22')  #Question: can a function name
print(x.plot_func())
#chi = x.chi()
#print("Chi: {}".format(chi))
